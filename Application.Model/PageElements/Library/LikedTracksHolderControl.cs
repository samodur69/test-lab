...
public class LikedTracksHolderControl(IElement root) : BaseElementControl(root)
{
    ...
    public TrackControl GetFirstTrack() => new(Root.FindElementByCss(ButtonCSS),
        int.Parse(Root.FindElementByCss(ButtonCSS).FindParent().GetAttribute("aria-rowindex")));
    ...
    [
        .. Root.FindElementByCss(ButtonCSS)
            ...
            .Select(elem =>
            {
                var index = int.Parse(elem.FindParent().GetAttribute("aria-rowindex"));
                return new TrackControl(elem, index);
            })
