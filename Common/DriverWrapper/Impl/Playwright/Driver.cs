...
{
    private readonly PlaywrightManager.Context _context = PlaywrightManager.Create(settings);

    public IElement FindElementByCss(string css, WaitingStrategy waitingStrategy = WaitingStrategy.NONE, string customArg0 = "",
        string customArg1 = "") =>
        FindWaitOnElement(new PlaywrightLocator(LocatorTypes.CSS_SELECTOR, css), waitingStrategy, customArg0, customArg1);

    public IElement FindElementByXPath(string xpath, WaitingStrategy waitingStrategy = WaitingStrategy.NONE, string customArg0 = "",
        string customArg1 = "") =>
        FindWaitOnElement(new PlaywrightLocator(LocatorTypes.XPATH, xpath), waitingStrategy, customArg0, customArg1);

    public IElement FindElement(ILocator locator, WaitingStrategy waitingStrategy = WaitingStrategy.NONE, string customArg0 = "",
        string customArg1 = "")
        => FindWaitOnElement(locator, waitingStrategy, customArg0, customArg1);

    public IEnumerable<IElement> FindElements(ILocator locator)
    {
        return _context.Page.Locator(LocatorConverter.Convert(locator)).AllAsync().Result
            .Select(IElement (loc) => new PlaywrightElement(loc.ElementHandleAsync().Result, _context.Page, locator))
            .ToList();
    }
    ...
    private PlaywrightElement FindWaitOnElement(ILocator locator, WaitingStrategy waitingStrategy = WaitingStrategy.NONE, string customArg0 = "",
        string customArg1 = "")
    {
        PlaywrightElement? foundElement = null;
        Exception lastException = new();

        var newLoc = LocatorConverter.Convert(locator);
        ...
        static bool WaitCriteria(WaitingStrategy strat, string customArg0, string customArg1, PlaywrightElement elem) =>
            elem switch
            {
                ...
                _ when strat == WaitingStrategy.ATTRIBUTE &&
                       string.Equals(elem.GetAttribute(customArg0), customArg1, StringComparison.OrdinalIgnoreCase) => true,
        ...
        bool WaitForElement()
        {
            try
            {
                ...
                if (!Waiter.WaitUntil(() =>
                    {
                        var loc = _context.Page
                            .WaitForSelectorAsync(newLoc, new PW.PageWaitForSelectorOptions { State = PW.WaitForSelectorState.Attached }).Result;
    ...
    public void ScrollDown(int byAmount = -1)
    {
        ...
        _context.Page.Mouse.WheelAsync(0, byAmount == -1
            ? windowSize.Height
            : byAmount).Wait();
    }
    ...
    {
        var screenshotPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, settings.ScreenshotPath);
        Directory.CreateDirectory(screenshotPath);

        var fileName = $"{string.Join("_", testName.Split(Path.GetInvalidFileNameChars()))}_{DateTime.Now:yyyy-MM-dd_HH-mm-ss}.png";
    ...
    public ICollection<Cookie> GetCookies() => _context.Page.Context.CookiesAsync().Result
    ...
    public void MoveCursorToElement(IElement element) => _context.Page.Mouse
        .MoveAsync((element as PlaywrightElement)!.Position.X, (element as PlaywrightElement)!.Position.Y).Wait();
    ...
    public void Click(Point pos) => _context.Page.Mouse.ClickAsync(pos.X, pos.Y).Wait();
    ...
    {
        _context.Page.Mouse.DblClickAsync(x, y).Wait();
    }

    public void DoubleClick(Point pos)
    {
        _context.Page.Mouse.DblClickAsync(pos.X, pos.Y).Wait();
    }

    public void MoveCursorTo(Point pos) => _context.Page.Mouse.MoveAsync(pos.X, pos.Y).Wait();
    public void MoveCursorTo(int x, int y) => _context.Page.Mouse.MoveAsync(x, y).Wait();
    public Point GetElementPos(IElement element) => (element as PlaywrightElement)!.Position;
}